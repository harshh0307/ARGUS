...
def _scan_source(scanner, source, filename, **kwargs):
    result = scanner.scan_source(source, filename, **kwargs)
...
def scan_js(source, filename="app.js"):
    return _scan_source(JsScanner(BASE), source, filename)
...
def test_scan_source_language_override():
    js = 'fetch("/repos/me")'
    usages = _scan_source(ApiScanner(BASE), js, filename="plain.txt", language="js")
    assert len(usages) == 1
