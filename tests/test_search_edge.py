...
def seed_entries(engine, count=3, vendor_slug="github", embedder=None):
    ...
    session.add(Vendor(slug=vendor_slug, name=vendor_slug))
    session.commit()
    entries = [
        {
            "title": f"Breaking: /items/{i}",
            "content": f"detail number {i}",
            "source_url": f"https://example.com/{i}",
        }
        for i in range(count)
    ]
    record_changelog_entries(session, vendor_slug, entries, embedder=embedder)
    session.commit()
    session.close()
...
def test_record_changelog_entries_empty_changes(tmp_path):
    ...
    session.commit()
    rows = record_changelog_entries(session, "github", [], embedder=None)
    session.commit()
    assert rows == []
...
def test_search_falls_back_to_keyword_when_embedder_fails(tmp_path):
    ...
    assert len(hits) >= 1
    assert hits[0][0].title == "Breaking: /items/1"
    session.close()
...
def test_search_no_embedding_rows_with_embedder_uses_keyword(tmp_path):
    ...
    session.commit()
    entries = [
        {"title": "Breaking: /kw", "content": "keyword hit", "source_url": "https://example.com/1"},
    ]
    ...
    assert len(hits) == 1
    assert hits[0][0].title == "Breaking: /kw"
    assert hits[0][1] == 1.0
    session.close()
...
def test_api_search_vendor_filter_and_limit(tmp_path):
    ...
    empty = client.get("/api/v1/search/changelog", params={"q": "number", "vendor": "stripe"}, headers=headers)
    assert empty.json() == []
...
def test_api_search_orders_by_score_desc(tmp_path):
    ...
    headers = register_and_login(client)
    response = client.get("/api/v1/search/changelog", params={"q": "detail"}, headers=headers)
    hits = response.json()
    scores = [h["score"] for h in hits]
    assert scores == sorted(scores, reverse=True)
    assert all(h["kind"].startswith("Breaking:") for h in hits)
